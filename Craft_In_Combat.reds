...
@replaceMethod(MenuHubGameController)
  private final func InitMenusData() -> Void {
    ...
    this.m_menusData = MenuDataBuilder.Make().Add(HubMenuItems.Crafting, HubMenuItems.None, n"crafting_main", n"ico_cafting", n"Gameplay-RPG-Skills-CraftingName").Add(HubMenuItems.Inventory, HubMenuItems.None, n"inventory_screen", n"ico_inventory", n"UI-PanelNames-INVENTORY").Add(HubMenuItems.Map, HubMenuItems.None, n"world_map", n"ico_map", n"UI-PanelNames-MAP", null, Cast<Bool>(GameInstance.GetQuestsSystem(this.m_player.GetGame()).GetFact(n"map_blocked"))).Add(HubMenuItems.Character, HubMenuItems.None, n"perks_main", n"ico_character", n"UI-PanelNames-CHARACTER").Add(HubMenuItems.Journal, HubMenuItems.None, n"quest_log", n"ico_journal", n"UI-PanelNames-JOURNAL").Add(HubMenuItems.Stats, HubMenuItems.Character, n"temp_stats", n"ico_stats_hub", n"UI-PanelNames-STATS").Add(HubMenuItems.Phone, HubMenuItems.Journal, n"phone", n"ico_phone", n"UI-PanelNames-PHONE").Add(HubMenuItems.Codex, HubMenuItems.Journal, n"codex", n"ico_data", n"UI-PanelNames-CODEX", CodexUserData.Make(CodexDataSource.Codex)).Add(HubMenuItems.Tarot, HubMenuItems.Journal, n"tarot_main", n"ico_tarot_hub", n"UI-PanelNames-TAROT", null, !Cast<Bool>(GameInstance.GetQuestsSystem(this.m_player.GetGame()).GetFact(n"q101_done"))).Add(HubMenuItems.Shards, HubMenuItems.Journal, n"shards", n"ico_shards_hub", n"UI-PanelNames-SHARDS", CodexUserData.Make(CodexDataSource.Onscreen)).Add(HubMenuItems.Backpack, HubMenuItems.Inventory, n"backpack", n"ico_backpack", n"UI-PanelNames-BACKPACK").Add(HubMenuItems.Cyberware, HubMenuItems.Inventory, n"cyberware_equip", n"ico_cyberware", n"UI-PanelNames-CYBERWARE");
